...
export const App: React.FC = () => {
  ...
  // Ensure document element is clean light mode
  useEffect(() => {
      ...
      <div className="flex-1 flex flex-col min-w-0 min-h-screen">
                  ...
                  {casesSubView === 'scenarios' ? (
                    <div className="space-y-4">
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        <div>
                          <h2 className="text-xl font-bold text-slate-900">
                            [{activeProject.key}] {activeModule.name} ({testScenarios.length} Test Scenarios)
                          </h2>
                          <p className="text-xs text-slate-500">
                            High-level QA repository with step-by-step execution instructions, positive/negative breakdown, and reference file importer.
                          </p>
                        </div>
                      ...
                      {testScenarios.length === 0 ? (
                        <div className="bg-white rounded-2xl p-12 text-center border border-slate-200">
                          ...
                          <h4 className="text-base font-extrabold text-slate-900">No Test Scenarios Stored Yet for {activeModule.name}</h4>
                          <p className="text-xs text-slate-500 mt-1 mb-4">
                            {canImportExport ? `Upload reference files (.csv, .xlsx, .json) or create manual test scenarios for ${activeModule.name}.` : `Only QA Lead or Admin roles can upload test scenarios.`}
                          </p>
                          {canImportExport && (
                            <button
                              onClick={() => setIsImporterOpen(true)}
                        ...
                        <TestCaseTable
                          ...
                          onDeleteTestCase={(key) => {
                            setCustomModuleScenarios(prev => ({
                              ...prev,
                              [activeModule.id]: (prev[activeModule.id] || []).filter(c => c.key !== key)
                            }));
                          }}
                          onBulkEditTestCases={(keys, updates) => {
                            const keySet = new Set(keys.map(k => k.trim().toUpperCase()));
                            setCustomModuleScenarios(prev => ({
                              ...prev,
                              [activeModule.id]: (prev[activeModule.id] || []).map(c => {
                                if (!keySet.has(c.key.trim().toUpperCase())) return c;
                                return {
                                  ...c,
                                  priority: updates.priority || c.priority,
                          ...
                          onBulkDeleteTestCases={(keys) => {
                            const keySet = new Set(keys.map(k => k.trim().toUpperCase()));
                            setCustomModuleScenarios(prev => ({
                              ...prev,
                              [activeModule.id]: (prev[activeModule.id] || []).filter(c => !keySet.has(c.key.trim().toUpperCase()))
                            }));
                          }}
        ...
        {isImporterOpen && activeModule && canManageCases && (
          <TestCaseImporter
            moduleName={activeModule.name}
            currentUser={currentUser}
            existingCases={casesSubView === 'scenarios' ? testScenarios : (customModuleCases[activeModule.id] || [])}
            mode={casesSubView === 'scenarios' ? 'scenarios' : 'cases'}
            onImportCases={casesSubView === 'scenarios' ? handleImportScenarios : handleImportCases}
